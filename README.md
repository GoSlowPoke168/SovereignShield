...
SovereignShield is a universally adaptable, crash-resistant security auditing three-agent AI pipeline that treats all incoming software updates and binaries as potentially hostile. It cross-examines vendor-declared SBOMs against deep-scan results, checks compile provenance against a zero-trust policy, and issues a confidence-scored, evidence-backed **BLOCK** or **ALLOW** verdict — automatically.
...
- **Resilient AI Pipeline:** Highly reliable agentic loop with custom token-limit management to handle large dependency trees without crashes.
- **Flexible Zero-Trust Policies:** Policies can be passed via CLI, auto-detected in the target directory, or fall back to internal defaults.
...
You can run SovereignShield against a live binary or use the built-in demo payload.

**Live Target Scan:**
```bash
python run.py --target /path/to/target/binary
```

**Live Target Scan with Overrides:**
```bash
python run.py --target /path/to/target/binary --policy /path/to/custom_policy.txt --vendor-sbom /path/to/vendor.sbom.json
```
...
After a successful run, reports are generated in `data/reports/`:

| File | Description |
|---|---|
| `inspection_report.txt` | Human-readable plain-text report |
| `inspection_report.html` | Styled HTML report for visual presentation |

Open the HTML report in any browser for the full visual dashboard.
...
sovereignshield/
├── data/
│   ├── mock_payloads/             # Built-in demo data
│   │   ├── vendor_sbom.json
│   │   ├── deep_scan_sbom.json
│   │   ├── update_metadata.json
│   │   └── zero_trust_policy.txt
│   └── reports/                   # Generated at runtime
│       ├── inspection_report.txt
│       └── inspection_report.html
├── sovereignshield/
│   ├── agents/
│   │   ├── decomposition_agent.py # NIST PO.1.1
│   │   ├── provenance_agent.py    # NIST PO.3.2
│   │   └── arbiter_agent.py       # NIST RV.1.3 + confidence feedback loop
│   ├── main.py                    # Pipeline orchestrator
│   ├── preprocessor.py            # Diff engine
│   ├── scanner.py                 # Live target deep scan & manifest discovery
│   └── watsonx_client.py          # IBM Granite 3.3 factory
├── run.py                         # CLI Launcher with pre-flight checks
...
All three agents use **IBM Granite 3.3 8B Instruct** (`ibm/granite-3-3-8b-instruct`) running on watsonx.ai.

The pipeline is tuned with low temperature (`0.05`) for deterministic, reproducible security verdicts, and optimized token limits to process extensive dependency graphs and manifests reliably.
...
### Phase 1: Live Binary Ingestion & Manifest Discovery
- **Action:** The target binary (or directory) undergoes a deep scan. SovereignShield features a "Smart Router" that automatically discovers relevant package manifests (e.g., `package.json`, `pom.xml`, embedded `sbom.json`) avoiding full, heavy directory parsing unless absolutely necessary.
...
- **Where RAG Happens:** For every hidden dependency detected, the pipeline automatically searches against a live ExploitDB database (via `vulnerability_fetcher.py`). It fetches known CVE records and cross-references them to the undisclosed packages. This **Retrieval-Augmented Generation** pattern guarantees the AI agents base their analysis on real-world threat intelligence rather than hallucinated or stale information.
...
- **Action:** Acting under NIST SP 800-218 PO.1.1, the Decomposition Agent (powered by IBM Granite 3.3) analyzes the diff provided by the pre-processor. Rather than parsing thousands of raw dependencies, it focuses solely on the discrepancies. It scores each undisclosed package assessing the exploit vector probability based on the injected RAG context.
- **Output:** A confidence-scored JSON payload cataloging all hidden dependencies and their associated severity.
- **Connection:** These findings are buffered for the final Arbiter decision.
...
- **Output:** The final definitive ALLOW/BLOCK label, a human-readable threat rationale, and actionable remediation instructions documented in both text and comprehensive HTML reports.
